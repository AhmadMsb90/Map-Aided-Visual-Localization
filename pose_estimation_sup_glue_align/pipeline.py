...
# Inject paths prior to evaluating neural framework package models
...
class SuperGluePosePipeline:
    def __init__(self):
        ...
        self.matching = Matching(config.SUPERGLUE_CONFIG).eval().to(config.DEVICE)

    def extract_ortho_patch(self, lon, lat):
        """Extracts a localized geographical window matching around estimated drone coordinate seeds."""
    ...
    def save_matches_visualization(self, img1_rs, patch_bgr, img2_rs, mkpts0, mkpts1, uav_img_num):
        """Generates a combined verification side-by-side matching canvas image file."""
        ...
        print("Saved tracking canvas match layout:", save_path)

    def process_frame(self, image_path, row_meta, uav_img_num):
        """Orchestrates flight orientation adjustments, descriptor matching execution, and dual-track PnP calculations."""
        ...
        # Warp canvas coordinates to counteract UAV structural flight yaw parameters
        ...
        # Isolate spatial map context references
        ...
        # Load tensors into SuperPoint network interface
        t0 = frame2tensor(img1_gray, config.DEVICE)
        t1 = frame2tensor(img2_gray, config.DEVICE)
        
        with torch.no_grad():
            pred = self.matching({"image0": t0, "image1": t1})
        ...
        if len(mkpts0) < config.MIN_GOOD_MATCHES:
            return None
            
        # Refine indices via baseline structural homography operations
        ...
        # Project validation outputs back onto un-warped flight perspective
        ...
        # Elevate features via ground elevation data structures
        obj_pts, img_pts = [], []
        ...
        # ------------------- Solution A: Anchor PnP Estimator -------------------
        ...
        # ------------------- Solution B: Telemetry Prior PnP Estimator -------------------
        ...
        # Perform explicit memory releases outside loop processing structures
        del t0, t1, pred
        torch.cuda.empty_cache()
        
        return {
            "kp_uav": len(kpts0), "kp_patch": len(kpts1), "raw_matches": len(matches), "good_matches": len(mkpts0),
            "good_ratio": len(mkpts0)/len(matches) if len(matches) > 0 else 0, "num_3d2d_corr": len(obj_pts_local),
