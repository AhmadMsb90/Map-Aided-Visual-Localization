...
* **Anchor-based Initialization:** A deterministic heuristic rotation matrix $\mathbf{R}_{\text{anchor}}$ is utilized to establish a stable down-looking nadir baseline configuration by swapping the horizontal coordinate components and flipping the vertical reference vector: $\mathbf{R}_{\text{map} \rightarrow \text{cam}}^{\text{init}} = \mathbf{R}_{\text{anchor}}$

* **Prior-based Initialization (IMU):** The orientation matrix $\mathbf{R}_{\text{imu}}$ derived from onboard telemetry is treated as directly absolute relative to the global map grid coordinate system ($\mathbf{R}_{\text{map} \rightarrow \text{body}}$). The seed prior is formulated by chaining this attitude state with the constant structural camera calibration displacement profile ($\mathbf{R}_{\text{boresight}}$): $\mathbf{R}_{\text{map} \rightarrow \text{cam}}^{\text{prior}} = \mathbf{R}_{\text{boresight}} \cdot \mathbf{R}_{\text{imu}}$
