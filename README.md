...
4. **Pose Optimization & Camera Position Recovery:**
   The solver refines the initialized pose guess against the local 3D-2D points, producing an optimized camera transformation matrix $\mathbf{R}_{\text{map} \rightarrow \text{cam}}^{\text{refined}}$ and translation vector $\mathbf{t}$. The spatial global camera location vector $\mathbf{C}$ is subsequently recovered in absolute metrics using spatial frame inversion:
